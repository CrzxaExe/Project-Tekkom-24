...
onMounted(() => {
    search.value.addEventListener("change", (event) => {
...
<template>
    ...
    <section class="w-full px-3 lg:px-24 -mt-18 lg:-mt-24">
        <div
            class="w-full py-3 bg-white dark:bg-back-900 px-4 lg:px-8 rounded-lg flex flex-row justify-between items-center">
            <h1 class="text-base lg:text-lg">Its all about art</h1>
            <div class="w-fill flex flex-row gap-2 items-center justify-end">
                <input ref="search" placeholder="Search" type="search"
                    ...
                    :class="[searchToggle ? 'w-[100%] py-0.5 px-1.5 lg:px-4' : 'w-[0%]']" />
                <i class='bx bx-search text-xl lg:text-2xl' @click="toggleSearchBtn"></i>
            </div>
        </div>


        <ul class="w-full grid grid-cols-2 md:grid-cols-6 lg:grid-cols-8 mt-6 lg:mt-16 py-4 gap-4 lg:gap-3 mx-auto lg:px-2 max-w-[68vw] lg:max-w-[74vw] dark-mode-transition">
            <GalleryItem  v-for="({ name, url }) in testGallery" :name :url />
        </ul>
    </section>
</template>